-- Compás Academy · Certificación Nivel 3 → Compás Evolution
-- Nivel 2 vigente es prerrequisito de emisión.

create or replace function public.verify_freelance_level3_certification(
  p_email text,
...
begin
  ...
  where cert.verification_code=v_code
    ...
    and course.slug='nivel-3-cartera-permanencia-crecimiento-compas'
...
revoke all on function public.verify_freelance_level3_certification(text,text)
  from public,authenticated;
grant execute on function public.verify_freelance_level3_certification(text,text)
  to anon,service_role;

create or replace function private.enforce_freelance_level3_prerequisite()
returns trigger
language plpgsql
security definer
set search_path to ''
...
begin
  ...
  if v_slug='nivel-3-cartera-permanencia-crecimiento-compas'
     and not exists(
       select 1
       from public.certificates cert
       join public.courses course2 on course2.id=cert.course_id
       where cert.user_id=new.user_id
         and cert.revoked_at is null
         and course2.slug='nivel-2-captacion-y-ventas-compas'
     ) then
    raise exception using
      errcode='22023',
      message='Nivel 2 certification is required before Nivel 3';
  end if;

  return new;
end $f$;

drop trigger if exists enforce_freelance_level3_prerequisite on public.certificates;
create trigger enforce_freelance_level3_prerequisite
before insert on public.certificates
for each row execute function private.enforce_freelance_level3_prerequisite();

create or replace function private.dispatch_freelance_level3_certification_event(
  p_event_id uuid
)
returns bigint
language plpgsql
...
begin
  ...
  if v_event.event_type<>'freelance_level3_certified' then
    raise exception 'Unsupported certification event';
  end if;

  select net.http_post(
    url:='https://qwxydjotwhniahaovrff.supabase.co/rest/v1/rpc/sync_evolution_commercial_academy_level3',
...
create or replace function private.queue_freelance_level3_certification()
returns trigger
language plpgsql
security definer
set search_path to ''
...
begin
  ...
  if not found or v_course.slug<>'nivel-3-cartera-permanencia-crecimiento-compas' then
    return new;
  end if;
  ...
  insert into public.academy_outbox_events(
    ...
    'freelance_level3_certified',
    'certificate',
    new.id,
    'freelance-level3-certificate:'||new.id::text,
    jsonb_build_object(
  ...
  begin
    perform private.dispatch_freelance_level3_certification_event(v_event_id);
...
drop trigger if exists sync_freelance_level3_certification on public.certificates;
create trigger sync_freelance_level3_certification
after insert on public.certificates
for each row execute function private.queue_freelance_level3_certification();
